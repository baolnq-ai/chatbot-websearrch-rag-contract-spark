...
AUTO_CREATE_ENV="${AUTO_CREATE_ENV:-true}"
...
FRONTEND_RUN_PORT="${FRONTEND_RUN_PORT:-6103}"
...
ensure_local_port_range() {
  ...
  if ! [[ "$value" =~ ^[0-9]+$ ]]; then
    echo "ERROR: $name must be a numeric local port, got: $value"
    exit 1
  fi

  if (( value < 6100 || value > 6150 )); then
    echo "ERROR: $name=$value is outside the local project port range 6100-6150"
    exit 1
  fi
}

validate_local_ports() {
  echo "[ports] Validating local project ports are within 6100-6150"
  ensure_local_port_range "NGINX_PUBLIC_PORT" "${NGINX_PUBLIC_PORT:-6101}"
  ensure_local_port_range "SERVER_PORT" "${SERVER_PORT:-6102}"
  ensure_local_port_range "FRONTEND_RUN_PORT" "${FRONTEND_RUN_PORT:-6103}"
  ensure_local_port_range "PARSER_HOST_PORT" "${PARSER_HOST_PORT:-6104}"
  ensure_local_port_range "EMBEDDING_HOST_PORT" "${EMBEDDING_HOST_PORT:-6105}"
  ensure_local_port_range "VLLM_HOST_PORT" "${VLLM_HOST_PORT:-6106}"
  ensure_local_port_range "PROM_COLLECTOR_HOST_PORT" "${PROM_COLLECTOR_HOST_PORT:-6107}"
  ensure_local_port_range "POSTGRES_HOST_PORT" "${POSTGRES_HOST_PORT:-6110}"
  ensure_local_port_range "PGBOUNCER_HOST_PORT" "${PGBOUNCER_HOST_PORT:-6111}"
  ensure_local_port_range "ADMINER_HOST_PORT" "${ADMINER_HOST_PORT:-6112}"
  ensure_local_port_range "QDRANT_HOST_PORT" "${QDRANT_HOST_PORT:-6113}"
  ensure_local_port_range "MINIO_API_HOST_PORT" "${MINIO_API_HOST_PORT:-6114}"
  ensure_local_port_range "MINIO_CONSOLE_HOST_PORT" "${MINIO_CONSOLE_HOST_PORT:-6115}"
  ensure_local_port_range "REDIS_HOST_PORT" "${REDIS_HOST_PORT:-6116}"
  ensure_local_port_range "REDIS_INSIGHT_HOST_PORT" "${REDIS_INSIGHT_HOST_PORT:-6117}"
  ensure_local_port_range "PROMETHEUS_HOST_PORT" "${PROMETHEUS_HOST_PORT:-6118}"
  ensure_local_port_range "NODE_EXPORTER_HOST_PORT" "${NODE_EXPORTER_HOST_PORT:-6119}"
  ensure_local_port_range "DCGM_EXPORTER_HOST_PORT" "${DCGM_EXPORTER_HOST_PORT:-6120}"
  ensure_local_port_range "SEARXNG_HOST_PORT" "${SEARXNG_HOST_PORT:-6121}"
  ensure_local_port_range "POSTGRES_EXPORTER_HOST_PORT" "${POSTGRES_EXPORTER_HOST_PORT:-6122}"
...
wait_for_http() {
  ...
  if [[ "$WAIT_FOR_READINESS" != "true" ]]; then
    echo "[ready] Skip $name ($url)"
    return 0
  fi

  echo "[ready] Waiting for $name: $url"
  while (( SECONDS < deadline )); do
    code="$(curl -sS -o /dev/null -w '%{http_code}' "$url" 2>/dev/null || true)"
    if [[ "$code" == "$expected" ]]; then
      echo "[ready] $name is ready ($code)"
      return 0
    fi
...
load_env_if_exists() {
  if [[ ! -f "$ENV_FILE" && "$ENV_FILE" == ".env" && "$AUTO_CREATE_ENV" == "true" && -f ".env.example" ]]; then
...
if [[ "$RUN_CODE_SERVICES" == "true" ]]; then
  wait_for_http "Backend" "http://127.0.0.1:${SERVER_PORT:-6102}/docs"
  wait_for_http "Frontend via nginx" "http://127.0.0.1:${NGINX_PUBLIC_PORT:-6101}/"
  wait_for_http "Parse-data" "http://127.0.0.1:${PARSER_HOST_PORT:-6104}/docs"
  wait_for_http "Embedding" "http://127.0.0.1:${EMBEDDING_HOST_PORT:-6105}/docs"
fi
